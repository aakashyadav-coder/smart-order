...
router.get("/analytics", authenticate, async (req, res, next) => {
  try {
    ...
    if (range === "24h") {
      ...
      bucketFn = (createdAt) => {
        const diff = Math.floor((now - createdAt) / (60 * 60 * 1000));
        return 23 - diff;
      };
    } else if (range === "30d") {
      // 30 daily buckets
      ...
      bucketFn = (createdAt) => {
        const diff = Math.floor((now - createdAt) / (24 * 60 * 60 * 1000));
        return 29 - diff;
      };
    } else {
      // 6 monthly buckets
    ...
    res.json({ range, labels, revenue, counts, totalRevenue, totalOrders });
  } catch (err) { next(err); }
});
